import image1 from "@/public/images/user/user-01.png";
import image2 from "@/public/images/user/user-02.png";
import { Testimonial } from "@/types/testimonial";

export const testimonialData: Testimonial[] = [
  {
    id: 1,
    name: "Chance ",
    designation: "Student, Class of 2024",
    image: image1,
    content:
      "Learning German at this school has been a transformative experience. I recently traveled to Germany with my family, and I was able to communicate confidently with locals, making the trip much more enjoyable. The lessons have truly expanded my horizons and practical language skills.",
  },
  {
    id: 2,
    name: "Angel",
    designation: "Student, Class of 2023",
    image: image2,
    content:
      "The German lessons have not only improved my language proficiency but also opened doors to understanding German culture. I now watch German films and read books in German, which has been both fun and educational. It has enriched my daily life by giving me a new perspective on global culture.",
  },
  {
    id: 3,
    name: "kevin",
    designation: "Student, Class of 2023",
    image: image1,
    content:
      "Thanks to the engaging German classes, I can now assist tourists in my city who speak German. Just last week, I helped a lost German family find their way, and they were so grateful. These lessons have given me more confidence to interact with people from different backgrounds.",
  },
  {
    id: 4,
    name: "Gasaro",
    designation: "Student, Class of 2024",
    image: image2,
    content:
      "Taking German has greatly boosted my career prospects. I recently interned at a company with international clients, and my German skills allowed me to contribute to meetings and translate documents. It’s amazing how these lessons have become so practical in my daily life and career path.",
  },
];
